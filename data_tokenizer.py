...
class DataTokenizer:
    ...
    def cleanup_chars2(self, word):
        while (ord(word[0]) < ord('a') or ord(word[0]) > ord('z')) and len(word) > 1:
            word = word[1:]
        while (ord(word[len(word) - 1]) < ord('a') or ord(word[len(word) - 1]) > ord('z')) and len(word) > 1:
            word = word[: -1]
        # if ord(specified_char) < ord('a') or ord(specified_char) > ord('z'):
        #    word = word[:index] + word[index + 1:]
        return word
